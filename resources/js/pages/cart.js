...
var Cart = {
    ...
    detectSubmit()
    {
        $(".js-paypal").click(function (event){
            event.preventDefault()
            let $this = $(this)
            let URL = $this.attr('data-url')
            $("#from_cart_register").attr('action', URL).submit();
        })
    },
    ...
    jsReductionQty()
    {
        $('.js-reduction').click(function (event) {
            ...
            if (number <= 1) {
                toast.warning("Number of products must be> = 1");
                return false;
            }
    ...
    jsIncreaseQty()
    {
        $('.js-increase').click(function (event) {
            ...
            let number = parseInt($input.val());
            if (number >= 10) {
                toast.warning("Each product can only buy up to 10 times / 1 purchase");
                return false;
            }
