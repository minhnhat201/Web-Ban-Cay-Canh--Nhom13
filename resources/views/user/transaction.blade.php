...
@section('content')
    <section>
        <div class="title">List of orders</div>
        <form class="form-inline">
            ...
            <div class="form-group" style="margin-right: 10px;">
                <select name="status" class="form-control">
                    ...
                    <option value="2" {{ Request::get('status') == 2 ? "selected='selected'" : "" }}>Being transported
                    </option>
                    <option value="3" {{ Request::get('status') == 3 ? "selected='selected'" : "" }}>Handed over
                    </option>
                    <option value="-1" {{ Request::get('status') == -1 ? "selected='selected'" : "" }}>Cancellation</option>
                </select>
            </div>
            <div class="form-group" style="margin-right: 10px;">
                        ...
                        <td style="text-align: center">
                            @if (!in_array($transaction->tst_status , [-1,2,3]) )
                                <a href="{{ route('get.user.transaction.cancel',$transaction->id) }}" 
                                   class="btn-xs label-danger" style="color: white"><i class="fa fa-save"></i> Cancel</a>
                           @endif
                        </td>
                    </tr>
                @endforeach
