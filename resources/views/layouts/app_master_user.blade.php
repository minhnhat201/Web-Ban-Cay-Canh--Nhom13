...
<head>
    ...
    <title>{{ strtolower($title_page ?? "Graduation thesis")   }}</title>
...
{{--        <p>Đăng nhập lần cuối <b>{{ get_time_login(Auth::user()->log_login)['time'] ?? "" }}</b></p>--}}
        <p class="user-info-item"><span>TK code</span> <span><b>#{{ Auth::user()->id }}</b></span></p>
         <p class="user-info-item"><span>Account balance</span> <span><b>{{ number_format(Auth::user()->balance,0,',','.') }} đ</b></span></p>
        <div class="content">
            <ul class="left-nav">
