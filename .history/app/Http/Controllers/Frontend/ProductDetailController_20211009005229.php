...
class ProductDetailController extends FrontendController
{
	public function getProductDetail(Request $request, $slug)
	{
		...
		if ($id) {
			...
			$event2 = Cache::remember('HOME.EVENT_3', 60 * 24 * 10, function () {
				return 
			});
	...
	private function getProductSuggests($categoriID)
	{
		$products = Cache::remember('PRODUCT_RELATE_'.$categoriID, 60 * 24 * 10, function () use ($categoriID) {
			return Product::where([
				...
				->orderByDesc('id')
				->select('id', 'pro_name', 'pro_slug', 'pro_sale', 'pro_number','pro_unit','pro_avatar', 'pro_price', 'pro_review_total', 'pro_review_star')
				->limit(12)
				->get();
		});
